...
void CFG::items()
{
  ...
  cout << "---------------------------------------" << endl;
  //list indices Sets of Items
  cout << " Indices of the sets: ";
...
{
  Action new_action;		// because of g++ bug
...
void CFG::parse(char *source_fn, ostream& out)
{
  ...
  switch (token.token==id) {
  case id: terminal = new Pid; break;
  case keyword: terminal = new Pkeyword; break;
  case eof: terminal = new Peof; break;
  }
  do {
    int state = p_stack.top();
	...
	p_stack.pop();
      ...
      int state2 = p_stack.top();
      //GSymbol* A = gen_symbol(); 
    }
...
template<>
GVariable<&Variables::Sp>& GVariable<&Variables::Sp>::gen()
{
  return (Self&)*new PSp;
}
...
template<>
GVariable<&Variables::S>& GVariable<&Variables::S>::gen()
{
  return (Self&)*new PS;
}
...
template<>
GVariable<&Variables::C>& GVariable<&Variables::C>::gen()
{
  return (Self&)*new PC;
}
