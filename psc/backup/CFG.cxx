...
void CFG::construct_parsing_tables()
{
  ...
  for (; i0!=j0; i0++) {
	...
	if ((prod.lhs == S) && (item.lookahead == eof_symbol)) {
	  Action act1;
	  act1.type == Action::accept;
	  action_table[I.index][item.lookahead] = act1;
	}
	else {
	  Action act2;
	  act2.type == Action::reduce;
	  act2.operand.prod = &prod;
	  action_table[I.index][item.lookahead] = act2;
	}
      }
    }
  }
...
void CFG::print_parsing_tables()
{
  ...
  for (;i2!=j2; i2++) {
	...
	cout << "reduce " << i3->second.operand.prod << endl;
